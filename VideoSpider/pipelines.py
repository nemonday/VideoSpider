...
class VideospiderPipeline(object):
    ...
    def insert_mysql_downlad(self, item):
        ...
        try:
            sql = "INSERT INTO tb_spider_video_copy1(vid, height, width, video_from, video_type, play_volume, " \
    ...
    def process_item(self, item, spider):
        if spider.name == HkSpider.name or spider.name == XngSpider.name or spider.name == XngzfSpider.name or spider.name == TdSpider.name :
            self.insert_mysql_downlad(item)

        elif spider.name == PpxSpider.name or spider.name == UcSpider.name or KySpider.name or spider.name:
