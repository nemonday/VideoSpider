...
class Iduoliao(object):
    ...
    @staticmethod
    def upload(url, img_url, filename, videofrom):
        if videofrom == "西瓜视频":
            # 传入视频下载地址，返回新的文件名字
            new_filename = IduoliaoTool.video_download(filename, url, ifdewatermark=True)
            # 获取视频的帧宽，帧高， 用于去水印定位
            ...
            if new_filename and size_filename and img_filename:
                ...
                dewatermark_name = IduoliaoTool.dewatermark(width, height, 20, 200, 55, 204, new_filename, filename)
                if dewatermark_name:
                    # oss上传视频
